...
- 서비스 애플리케이션과 통신하는 카프카 클러스터 외에 **재해 복구를 위해, 임시 카프카 클러스터를 하나 더 구성할 수 있다**
  - 액티브 클러스터 : 서비스 애플리케이션들이 직접 통신하는 카프카 클러스터
  - 스탠바이 클러스터 : 나머지 1개 카프카 클러스터
- 각 클러스터 간의 구역을 완벽히 분리함으로써, 액티브 클러스터의 장애에 대응할 수 있다
  - 자연재해, 기술적 재해, 인간에 의한 재해와 같은 재해에 대응하기 가장 좋은 방법이 **물리적 공간을 분리**하는 것이다
![alt text](image-21.png)
- 미러메이커2는 액티브 클러스터 -> 스탠바이 클러스터에 데이터를 복제할 때 복제 지연되는 현상인 `복제 랙` 이 발생할 수 있다. 
  - 따라서 스탠바이 클러스터에 액티브 클러스터의 모든 정보가 복제되지 않았을 가능성도 존재한다
  > 복제 랙 원인
  > 1. 네트워크 지연 - 클러스터 간 물리적 거리, 네트워크 불안정 등
  > 2. MirrorMaker2 처리 병목 - Task 수 부족, 내부 큐 포화, 처리량 초과
  > 3. 메시지 생성 속도 > 복제 속도 - 생산자가 너무 빠르게 메시지를 보낼 경우

#### 액티브 - 액티브 (Active-active) 클러스터 운영
![alt text](image-22.png)
- 서비스 애플리케이션의 통신 지연을 최소화하기 위해 **2개 이상의 클러스터를 두고 서로 데이터를 미러링하면서 사용**할 수 있는데 이 때 액티브-액티브 클러스터 운영하는 것이 하나의 방법이다
  - 물리적으로 아주 멀리 떨어진 곳에 있는 2명의 유저의 데이터를 저장하고 사용하는 방법으로, 각 지역마다 클러스터를 두고 필요한 데이터만 복제하여 사용하는 방법을 사용할 수 있다
  - 해당 지역의 유저가 관련 데이터를 조회할 때 데이터의 지연을 줄일 수 있다

#### 허브 앤 스포크 (Hub and spoke) 클러스터 운영
![](image-23.png)
- 각 팀의 카프카 클러스터의 데이터를 **한 개의 카프카 클러스터에 모아 데이터 레이크로 사용**하고 싶다면 허브 앤 스포크 방식의 클러스터 구성이 한가지 방안이 될 수 있다
- 미러메이커2를 사용하여 각 팀에서 사용하는 카프카 클러스터에 존재하는 데이터를 수집하고 데이터 레이크용 카프카 클러스터에서 가공, 분석하여 가치 있는 데이터를 찾아낼 수 있다.