...
### 5.2.5 상용 인프라 아키텍처
- 최소한으로 서버 지표 수집 파이프라인 인프라를 구축하여 안전하게 운영하고 싶다면 아래와 같이 구성할 수 있다.
  - 카프카 클러스터 : 3개 이상의 브로커로 구성
...
```
### 5.3.2 기능 정의
- 미러메이커2를 운영할 경우 고려해야 할 점
  - **각 클러스터의 보안** : 클러스터에 보안 설정이 되어 있을 경우 권한을 획득하고 접근할 수 있도록 설정이 필요
  - **미러링 토픽 이름** : 정규식으로 토픽을 표현할 수 있으므로 미러링해야 하는 토픽이 많을 경우 정규식으로 선언하면 미러메이커2를 재시작할 필요없이 토픽이 생성되는 대로 모두 미러링할 수 있다.
...
- 분산 모드 커넥트를 운영 중이라면 별도로 미러메이커2를 위한 서버를 구축하지 않아도 된다.
  <img width="295" height="220" alt="스크린샷 2025-11-02 오후 11 42 27" src="https://github.com/user-attachments/assets/a6c90edc-d0e9-47c7-ba49-dac8452135a7" />
  - 미러멩커2는 커넥터로도 동작하도록 만들어졌다. 카프카 2.5.0에서 분산 모드 카프카 커넥트를 실행하면 미러링을 위한 커넥터가 준비되어 있으므로 따로 미러메이커2 프로세스를 띄울 필요 없이 분산 모드 커넥트에서 미러메이커2 커넥터를 운영하는 방법도 있다.
